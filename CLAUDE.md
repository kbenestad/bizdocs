...
| `dashboard/`      | Shared profile + a log of documents generated by invoice/reimburse/timesheet, exported/imported as one backup file | — (JSON export/import) |
| `themeselector/`  | Preview any theme from [mdcms](https://github.com/kbenestad/mdcms)'s `themes/` folder mapped onto a live bizdocs app | — (preview tool, no output) |
...
dashboard/      index.html · config.yml · assets/ (favicons)
themeselector/  index.html · config.yml · assets/ (favicons)
...
- **localStorage keys.** Theme = `kb-theme`, font scale = `kb-font-scale`
  ...
  invoice `inv_data_v1` / `inv_lines_v1` / `inv_generated_v1` /
  `inv_records_v1`; reimburse `reimb-state` / `reimb-staff` / `reimb-lang` /
  ...
  edited/viewed from the dashboard app: `kb-profile-v1` (the user's own
  identity/bank details — invoice/reimburse/timesheet pre-fill blank fields
  from it) and the `*_records`/`*records_v1` keys above (one per document
  app, a log of generated documents' metadata, not the PDFs themselves).
...
- **The previewed app is a live iframe, not a copy.** Its "App" dropdown
  ...
  every other route serves — so it never goes stale and there's nothing to
  keep in sync when those apps change.
- **Theme data is fetched live from mdcms, not vendored.** The "Theme
  category"/"Theme" dropdowns call the GitHub Contents API against
  `kbenestad/mdcms`'s `themes/` folder, then fetch the chosen file's raw YAML
  and parse it with the same `js-yaml` every app already loads. No theme
  values are copied into bizdocs, so a new mdcms theme shows up here with no
  bizdocs-side change. This is why its CSP `connect-src` additionally allows
